(()=>{
  const ASSET='assets/';
  const STEPS=[
    {n:1,title:'Acción positiva',short:'El alumno completa un reto, gana un logro o realiza un canje.',icon:'✓',tag:'Reto o canje'},
    {n:2,title:'Constancia segura',short:'Wolves crea una constancia pública sin guardar emociones ni comentarios.',icon:'◆',tag:'Registro limpio'},
    {n:3,title:'Firma SHA-256',short:'La acción se transforma en un hash para comprobar que no fue alterada.',icon:'#',tag:'Criptografía'},
    {n:4,title:'Libro Mayor',short:'El bloque queda unido al anterior como una cadena de evidencias.',icon:'▣',tag:'Bloque protegido'}
  ];
  const PRIVACY=[
  ...
  function blockMeta(action=''){
    const text=action.toLowerCase();
    if(text.includes('canje'))return {icon:'↗',label:'Canje validado',type:'store'};
    if(text.includes('logro'))return {icon:'★',label:'Logro validado',type:'achievement'};
    if(text.includes('recompensa'))return {icon:'✦',label:'Recompensa validada',type:'reward'};
    return {icon:'✓',label:'Reto validado',type:'challenge'};
  }
  function latestBlock(){
    const list=chainList();
    if(!list.length){
      return `<div class="bc-current-empty"><div class="bc-empty-icon">▣</div><h3>Aún no hay bloques generados</h3><p>Usa el botón de demostración para ver cómo Wolves registra una constancia sin exponer información emocional.</p></div>`;
    }
    const block=list[0];
    const meta=blockMeta(block.action);
    return `<article class="bc-current-block bc-current-block-icons">
      <div class="bc-current-symbol ${meta.type}"><span>${meta.icon}</span><b>${meta.label}</b><small>Bloque #${block.number}</small></div>
  ...
  function rows(){
    ...
    return list.map(block=>{const meta=blockMeta(block.action);return `<article class="bc-ledger-item no-image ${meta.type}">
      <div class="bc-ledger-icon"><span>${meta.icon}</span></div>
      <div class="bc-ledger-main"><div><b>#${block.number}</b><span>${meta.label}</span></div><h3>${block.action}</h3><p>${block.date}</p></div>
  ...
  window.chain=function blockchainExplorerUpgrade(){
    ...
    return `<section class="bc-shell">
      ...
      <article class="bc-flow-card">
        ...
        <div class="bc-step-grid">${STEPS.map(step=>`<div class="bc-step-card icon-only"><div class="bc-step-number">${step.n}</div><div class="bc-step-icon-panel"><span>${step.icon}</span><small>${step.tag}</small></div><h3>${step.title}</h3><p>${step.short}</p></div>`).join('')}</div>
      </article>
