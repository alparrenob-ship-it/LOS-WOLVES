(()=>{
  ...
  function avatar(color,type,nft){
  ...
  function profileIdentity(){
    ...
    return `<section class="identity-profile-grid">
      <article class="rpg-card identity-avatar-card">
        ...
        <div class="identity-avatar-orbit">${avatar(aura.color,u.avatarType,chosenNft)}</div>
        <h2>${u.profileName}</h2>
        <p class="student-rank">${u.avatarType==='loba'?'Loba':'Lobo'} Wolves · ${aura.label}</p>
        <p class="student-motto">“${u.motto}”</p>
      </article>
      <article class="rpg-card identity-editor-card">
        <div class="section-title-row"><div><h2>Personalización de la manada</h2><p>Adapta tu identidad escolar, avatar, color y frase motivacional.</p></div><span class="secure-chip">Datos seguros</span></div>
        <div class="identity-form-grid">
          ...
          <label>Avatar Wolves<div class="avatar-toggle"><button class="avatar-kind ${u.avatarType==='lobo'?'active':''}" data-avatar="lobo">Lobo</button><button class="avatar-kind ${u.avatarType==='loba'?'active':''}" data-avatar="loba">Loba</button></div></label>
      ...
      <article class="rpg-card full-width identity-nft-card">
        <div class="section-title-row"><div><h3>Accesorios NFT desbloqueables</h3><p>Prueba cada accesorio en tu ${u.avatarType==='loba'?'loba':'lobo'} y equípalo cuando completes los 5 retos.</p></div><span class="season-chip">${(S.challenges||[]).length}/5 retos</span></div>
  ...
  function bindIdentity(){
    ...
    document.querySelectorAll('.avatar-kind').forEach(btn=>btn.onclick=()=>{currentUser().avatarType=btn.dataset.avatar;persist();render();});
